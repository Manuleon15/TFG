...
# Leemos la base de datos con los precios diarios del sector Bonos
df = pd.read_csv('/Users/manuel/Desktop/pythonProject/precios_sector_bonos.csv', index_col='Date', parse_dates=True)
...
plt.title('Datos Entrenamiento y Test Sector Bonos')
plt.legend()
plt.show()
...
plt.title('Sector Bonos Predicción Precios')
plt.xlabel('Fecha')
plt.ylabel('Precio Sector Bonos')
plt.legend()
plt.grid(True)
plt.show()
...
model = SARIMAX(history, order=(3, 1, 2), exog=exog_history)
model_fit = model.fit()
...
# los residuos parecen comportarse como ruido blanco hasta el retraso 40.
