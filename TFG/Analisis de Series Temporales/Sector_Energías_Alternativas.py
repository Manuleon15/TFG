...
# Leemos la base de datos con los precios diarios del sector Energías Alternativas
df = pd.read_csv('/Users/manuel/Desktop/pythonProject/precios_sector_energia.csv', index_col='Date', parse_dates=True)
...
plt.title('Datos Entrenamiento y Test Sector Energías Alternativas')
plt.legend()
plt.show()
...
plt.title('Sector Energías Alternativas Predicción Precios')
plt.xlabel('Fecha')
plt.ylabel('Precio Sector Energías Alternativas')
plt.legend()
plt.grid(True)
plt.show()
...
model = SARIMAX(history, order=(2, 1, 2), exog=exog_history)
model_fit = model.fit()
