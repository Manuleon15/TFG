...
# Leemos la base de datos con los precios diarios del sector financiero
df = pd.read_csv('/Users/manuel/Desktop/pythonProject/precios_sector_finanzas.csv', index_col='Date', parse_dates=True)
...
plt.title('Datos Entrenamiento y Test Sector Financiero')
plt.legend()
plt.show()
...
plt.title('Sector Financiero Predicción Precios')
plt.xlabel('Fecha')
plt.ylabel('Precio Sector Financiero')
plt.legend()
plt.grid(True)
plt.show()
...
model = SARIMAX(history, order=(0, 1, 2), exog=exog_history)
model_fit = model.fit()
