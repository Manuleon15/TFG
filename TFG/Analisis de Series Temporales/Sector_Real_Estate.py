...
# Leemos la base de datos con los precios diarios del sector Real Estate
df = pd.read_csv('/Users/manuel/Desktop/pythonProject/precios_sector_rs.csv', index_col='Date', parse_dates=True)
...
plt.title('Datos Entrenamiento y Test Sector Real Estate')
plt.legend()
plt.show()
...
plt.title('Sector Real Estate Predicción Precios')
plt.xlabel('Fecha')
plt.ylabel('Precio Sector Real Estate')
plt.legend()
plt.grid(True)
plt.show()
...
model = SARIMAX(history, order=(1, 1, 4), exog=exog_history)
model_fit = model.fit()
