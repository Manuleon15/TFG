...
# Leemos la base de datos con los precios diarios del sector Salud
df = pd.read_csv('/Users/manuel/Desktop/pythonProject/precios_sector_salud.csv', index_col='Date', parse_dates=True)
...
plt.title('Datos Entrenamiento y Test Sector Salud')
plt.legend()
plt.show()
...
plt.title('Sector Salud Predicción Precios')
plt.xlabel('Fecha')
plt.ylabel('Precio Sector Salud')
plt.legend()
plt.grid(True)
plt.show()
...
model = SARIMAX(history, order=(3, 1, 4), exog=exog_history)
model_fit = model.fit()
