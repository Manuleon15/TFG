...
# Leemos la base de datos con los precios diarios del sector tecnológico
df = pd.read_csv('/Users/manuel/Desktop/pythonProject/precios_sector_tecnologia.csv', index_col='Date', parse_dates=True)
...
plt.title('Datos Entrenamiento y Test Sector Tecnológico')
plt.legend()
plt.show()
...
plt.title('Sector Tecnología Predicción Precios')
plt.xlabel('Fecha')
plt.ylabel('Precio Sector Tecnología')
plt.legend()
plt.grid(True)
plt.show()
...
model = SARIMAX(history, order=(1, 1, 2), exog=exog_history)
model_fit = model.fit()
